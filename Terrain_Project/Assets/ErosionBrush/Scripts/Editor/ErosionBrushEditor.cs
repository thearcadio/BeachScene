...
public class ErosionBrushEditor : Layout
{
	...
	public override void OnInspectorGUI () 
	{
		...
		if (script.guiShowTextures)
		{
			...
			Label("Crag", width:70);
			Slider<float>(ref preset.foreground.opacity, width:width-130, max:4);
			...
			Slider<float>(ref preset.background.opacity, width:width-130, max:4);
		...
		if (script.guiShowAbout)
		{
			...
			Par(); Label("Erosion Brush v1.4_u5");
	...
	public void OnSceneGUI ()
	{
		...
		if (Event.current.type == EventType.MouseUp && Event.current.button == 0) 
			{ oldBrushPos = new Vector3(-65000,0,-65000); script.terrain.ApplyDelayedHeightmapModification(); }
		...
		//refresh terrain lods on mouse up
		if (Event.current.type==EventType.MouseUp) { script.terrain.ApplyDelayedHeightmapModification(); Debug.Log("Delayayed Apply"); }
