...
namespace ErosionBrushPlugin 
{
	...
	public struct Coord
	{
		...
		public IEnumerable<Coord> CircularPerimeter (int dist) //a circular square border around center
		{
			for (int iz=0; iz>-dist; iz--) yield return new Coord(x-dist, z+iz);
			for (int ix=-dist; ix<dist; ix++) yield return new Coord(x+ix, z-dist);
			for (int iz=-dist; iz<dist; iz++) yield return new Coord(x+dist, z+iz);
			for (int ix=dist; ix>-dist; ix--) yield return new Coord(x+ix, z+dist);
			for (int iz=dist; iz>0; iz--) yield return new Coord(x-dist, z+iz);
		}
		...
		public IEnumerable<Coord> CircularArea (int maxDist)
		{
			...
			for (int i=0; i<maxDist; i++)
				foreach (Coord c in CircularPerimeter(i)) yield return c;
		}
